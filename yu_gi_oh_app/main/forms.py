...
class DeckForm(FlaskForm):
    """Form for adding/updating a Deck."""
...
class CardForm(FlaskForm):
    """Form for adding/updating a Card."""
    name = StringField('Card Name', validators=[DataRequired()])
    card_type = SelectField('Card Type', choices=CardType.choices())
    photo_url = StringField('Photo URL', validators=[URL()])
    deck = QuerySelectField('Deck', query_factory=lambda: Deck.query, allow_blank=False, get_label='name')
    submit_button = SubmitField('Submit')
