...
class MainTests(unittest.TestCase):
    ...
    def test_book_detail_logged_out(self):
        ...
        self.assertIn(book1.title, response_text)
        self.assertIn(str(book1.publish_date), response_text)
        self.assertIn(book1.author.name, response_text)
    ...
    def test_create_author(self):
        ...
        post_data = {
            'name': 'Sarah Williams',
            'biography': 'Author from over one hundred years ago.',
            'books': ['Twilight Hours']
        }
        response = self.app.post('/create_author', data=post_data)
        self.assertEqual(response.status_code, 302)

        # TODO: Verify that the author was updated in the database
        created_author = Author.query.filter_by(name='Sarah Williams').one()
        self.assertIsNotNone(created_author)
        self.assertEqual(created_author.name, 'Sarah Williams')
    ...
    def test_create_genre(self):
        ...
        post_data = {
            'name': 'Horror',
            'books': ['Nightmare on Elm Street']
        }
        response = self.app.post('/create_genre', data=post_data)
        self.assertEqual(response.status_code, 302)
    ...
    def test_unfavorite_book(self):
        ...
        response2 = self.app.post('/unfavorite/1')
        ...
        user = User.query.filter_by(username='me1').one()
        self.assertIsNotNone(user)
        self.assertEqual(len(user.favorite_books), 0)